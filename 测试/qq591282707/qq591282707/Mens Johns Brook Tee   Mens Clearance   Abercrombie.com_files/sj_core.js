...
function SjURI(str) {
    ...
    this.scheme = result[2] || null;
    this.authority = result[4] || null;
    this.path = result[5] || null;
    this.query = result[7] || null;
    this.fragment = result[9] || null;
}

// RFC3986 5.2.2. Transform References
SjURI.prototype.resolve = function (baseURI) {
	//RFC3986 5.2.3 Merge Paths
	function merge(baseURI, inPath){
		var dirname = /^(.*)\//;
		if (baseURI.authority && !baseURI.path){
			return "/" + inPath;
		}else if(baseURI.path){
			if (baseURI.path.match(dirname)){
				return baseURI.path.match(dirname)[0] + inPath;
			}else{
				//return baseURI.path +"/" + inPath;
				return inPath;
			}
		}
		return inPath;
	...
	function remDotSeg(path){
	var dblDot = /\/((?!\.\.\/)[^\/]*)\/\.\.\//;
		if (!path) return "";
		var newpath = path.replace(/\/\.\//g, '/');
		newpath = newpath.replace(/\/\.$/, '/');
		while(newpath.match(dblDot)){
			newpath = newpath.replace(dblDot, '/');
		}
		newpath = newpath.replace(/\/([^\/]*)\/\.\.$/, '/');
		while(newpath.match(/\/\.\.\//)){
			newpath = newpath.replace(/\/\.\.\//, '/');
	...
	var resolvedURI = new SjURI();
	if (this.scheme){
		resolvedURI.scheme = this.scheme;
		resolvedURI.authority = this.authority;
		resolvedURI.path = remDotSeg(this.path);
		resolvedURI.query = this.query;
	}else{
		if (this.authority){
			resolvedURI.authority = this.authority;
			resolvedURI.path = remDotSeg(this.path);
			...
			if (!this.path){
				resolvedURI.path = baseURI.path;
				if (this.query){
					resolvedURI.query = this.query;
				}else{
					resolvedURI.query = baseURI.query;
				}
			}else{
				if (this.path.charAt(0) === '/'){
					resolvedURI.path = remDotSeg(this.path);
				}else{
					resolvedURI.path = merge(baseURI, this.path);
					resolvedURI.path = remDotSeg(resolvedURI.path);
				}
				resolvedURI.query = this.query;
			}
			resolvedURI.authority = baseURI.authority;
		}
		resolvedURI.scheme = baseURI.scheme;
	}
	resolvedURI.fragment = this.fragment;
	return resolvedURI;
};

SjURI.prototype.toString = function () {
    var str = "";
	str = ((this.scheme)?this.scheme + ":":"") +
