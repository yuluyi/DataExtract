...
SjZoomNav.prototype.setViewer = function(inObj){
	...
	if (this.navObj){
		...
		if ((this.navObj.navloadURL) && (this.navObj.imgServerWidth) && (this.navObj.imgServerHeight)){
			...
			this.navObj.navloadURL +=sjPBreak(this.navObj.navloadURL) + 'scl='+(this.navObj.navDefaultScl)+this.navObj.idStr;
			this.dblnavImage.visible(false);
...
SjZoom.prototype.initialising = function (x,y,scale,defaultScl) {
	...
	//create url
		...
		this.loadURL = this.mainURL+sjPBreak(this.mainURL)+'rect='+
							Math.round(0)+','+
							Math.round(0)+','+
							Math.round(this.imgServerWidth/this.defaultScl)+','+
							Math.round(this.imgServerHeight/this.defaultScl);
		this.navloadURL = this.mainURL;
	...
	//load image
		...
		if (this.navLayer){
			...
			this.navloadURL += sjPBreak(this.navloadURL) + 'scl='+(this.navDefaultScl)+this.idStr;
...
//		if (!window.Touch) sjSetCursor('wait');
		if (!("ontouchstart" in window)) sjSetCursor('wait');
...
//		if (!window.Touch) sjSetCursor('default');
		if (!("ontouchstart" in window)) sjSetCursor('default');
...
SjZoom.prototype.updateForeImage = function (inURL){
	...
	var tempURL = res.url + sjPBreak(res.url)+'rect='+Math.round(res.x/res.scl)+','+Math.round(res.y/res.scl)+','+Math.round(res.w/res.scl)+','+Math.round(res.h/res.scl)+
							'&scl='+(res.scl)+this.idStr;
	var timageURL = "";
	var tinURL = "";
	var recURL = "";
	var	requered = false;
	var tserver = this.serverURL || "/is/image/";
	if (typeof inURL != "undefined"){
		timageURL = this.foreImage.imageElm.src.substr(this.foreImage.imageElm.src.indexOf(tserver)+tserver.length);//img
		tinURL = inURL.substr(inURL.indexOf(tserver)+tserver.length);
		if (timageURL != tinURL){
			requered = true;
			recURL = inURL;
		}
	}else{
		tinURL = tempURL.substr(tempURL.indexOf(tserver)+tserver.length);
		timageURL= this.foreImage.imageElm.src.substr(this.foreImage.imageElm.src.indexOf(tserver)+tserver.length);//img
